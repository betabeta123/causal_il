...
data = pd.read_csv('./top_8_features_and_labels.csv')
...
a = 0  # 起始值
b = 3  # 终止值
new_data_do.iloc[:, n] = a + (b - a) * np.random.rand(len(data))#索引是从0开始的
...
AUG_upsample_dataset.to_csv('AUG_upsample_dataset_5.csv', index=False)
print(f'数据集 AUG_upsample_dataset 的行数和列数：{AUG_upsample_dataset.shape}')
